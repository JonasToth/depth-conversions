...
if ! ${exe} \
   -i "flexion-{}.png" \
   -o "brisk-size-{}.feature" \
   -s 0 -e 1 \
   detector --kp-size-threshold 80. brisk \
   descriptor brisk ; then
    print_error "BRISK-Detection filtering by keypoint size did not work"
    exit 1
fi
if  [ ! -f brisk-size-0.feature ] || \
    [ ! -f brisk-size-1.feature ]; then
    print_error "Did not create expected output files."
    exit 1
fi
if  [ "$(du -k brisk-0.feature | cut -f1)" -le "$(du -k brisk-size-0.feature | cut -f1)" ]; then
...
if ! ${exe} \
   -i "flexion-{}.png" \
   -o "brisk-response-{}.feature" \
   -s 0 -e 1 \
   detector --kp-response-threshold 200. brisk \
   descriptor brisk ; then
    print_error "BRISK-Detection filtering by keypoint response did not work"
    exit 1
fi
if  [ ! -f brisk-response-0.feature ] || \
    [ ! -f brisk-response-1.feature ]; then
    print_error "Did not create expected output files."
    exit 1
fi
if  [ "$(du -k brisk-0.feature | cut -f1)" -le "$(du -k brisk-response-0.feature | cut -f1)" ]; then
    print_error "Filter keypoints must produce less data."
    exit 1
fi
